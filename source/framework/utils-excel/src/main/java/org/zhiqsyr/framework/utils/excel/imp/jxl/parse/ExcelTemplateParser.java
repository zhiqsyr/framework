...
import org.zhiqsyr.framework.utils.excel.imp.jxl.parse.model.ColumnWidthDefine;
import org.zhiqsyr.framework.utils.excel.imp.jxl.parse.model.MergedCell;
...
public class ExcelTemplateParser {
	...
	private Integer startRow;	// 开始行号
	private Integer endRow;		// 结束行号
	private Integer startCol;	// 开始列号
	private Integer endCol;		// 结束列号
	
	private boolean needParseComment = true;	// 是否需要解析标注信息
	private boolean needParseStyle = true;		// 是否需要解析样式信息
	
	private ParseMode parseMode = ParseMode.rowAfterRow;	// 数据解析方式
	...
	private void extractTemplateContent(Sheet sheet, TableModel table) {
		...
		if (parseMode == ParseMode.colAfterCol) {	
			outerStart = getStartCol() <= 1 ? 0 : getStartCol() - 1;
			outerEnd = getEndCol() < 1 ? getSheet().getColumns() : getEndCol();
		// 按行解析
		} else {									
			outerStart = getStartRow() <= 1 ? 0 : getStartRow() - 1;
			outerEnd = getEndRow() < 1 ? getSheet().getRows() : getEndRow();
		}
		...
		for (; outerStart < outerEnd; outerStart++) {
			...
			// 当前行(或列)的合并单元格
			List<MergedCell> rowRanges = parseMode == ParseMode.colAfterCol ? 
					JxlUtil.extractColMergedCells(outerStart, ranges) : 
					JxlUtil.extractRowMergedCells(outerStart, ranges);
			...
			// 按列解析
			if (parseMode == ParseMode.colAfterCol) {
				innerStart = getStartRow() <= 1 ? 0 : getStartRow() - 1;
				innerEnd = getEndRow() < 1 ? getSheet().getRows() : getEndRow();
			} else {
				innerStart = getStartCol() <= 1 ? 0 : getStartCol() - 1;
				innerEnd = getEndCol() < 1 ? getSheet().getColumns() : getEndCol();
			}
			// 解析当前行的单元格
			for (; innerStart < innerEnd; innerStart++) {
				// 如果是按列解析，则outerStart代表列，否则代表行
				Cell cell = parseMode == ParseMode.colAfterCol ? 
						sheet.getCell(outerStart, innerStart) : 
						sheet.getCell(innerStart, outerStart);
	...
	private void buildColumnDefine(Sheet sheet, TableModel table) {
		...
		int c = getStartCol() <= 1 ? 0 : getStartCol() - 1;
		int end = getEndCol() < 1 ? getSheet().getColumns() : getEndCol();
		for (; c < end; c++) {
	...
	private Workbook loadWorkbook() {
		...
		wbs.setSuppressWarnings(true);  //不显示Warning信息 ，Excel使用定义名称会报Warning
		
		Workbook workbook = null;
		try {
			workbook = Workbook.getWorkbook(resource, wbs);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		return workbook;
	...
	public Integer getStartRow() {
		return startRow == null ? 0 : startRow;
	}
	...
	public Integer getStartCol() {
		return startCol == null ? 0 : startCol;
	}
